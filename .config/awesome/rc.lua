...
-- beautiful.init("/home/wellska1/.config/awesome/theme/theme.lua")
...
tags = {
    ...
    -- names = { 'q', 'w', 'e', 'r', 'a', 's', 'd', 'f', 'email' },
    layout = { layouts[1], layouts[1], layouts[1], layouts[1],
               layouts[1], layouts[1], layouts[1], layouts[1],
               layouts[9], layouts[1], layouts[1], layouts[9] } 
}
...
-- Initialize widget
-- cpuw = widget({ type = "textbox" })
-- Register widget
-- vicious.register(cpuw, vicious.widgets.cpu, "$1%")
...
-- Configure obvious clock
--obvious.clock.set_editor("xterm -e vim")
obvious.clock.set_editor(editor_cmd)
-- obvious.clock.set_shortformat("%x %H:%M ") -- 11/29/10 22:19
obvious.clock.set_shortformat("%x %I:%M %p ") -- 11/29/10 10:19 pm
obvious.clock.set_longformat("%x %I:%M %p ") -- 11/29/10 10:19 pm
...
-- mywibox[1].widgets = { mytextbox }
-- mystatusbar = wibox({ position = "bottom", name = "mystatusbar", screen = 1 })
...
-- mytitlebartitle.text = "Hello focused app"
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey, "Shift"   }, "t", function () awful.util.spawn(terminal .. " -e todo") end)
...
-- for i,v in ipairs(tags.names) do
...
client.add_signal("manage", function (c, startup)
    ...
    -- Enable using mouse click to focus
    -- c:add_signal("mouse::click", function(c)
    c:add_signal("mouse::enter", function(c)
    -- Or, to enable sloppy focus: mouse::enter
